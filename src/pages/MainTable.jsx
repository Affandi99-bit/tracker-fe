...
const DataTable = ({
  ...
  const canShare = useHasPermission("share");
  const canKanban = useHasPermission("kanban");
  const canEquip = useHasPermission("equipment");
  const canOvertime = useHasPermission("overtime");
  const isFinance = useHasPermission("finance");
          ...
          return (
            ...
            >
              ...
              <td className=" text-xs">
                <div className="w-full flex flex-wrap px-2 gap-3 justify-start items-center">
                  {/* Edit */}
                  {canEdit && (
                  ...
                  {/* Overtime */}
                  {canOvertime && (
                    <Tooltip position="bottom" content={"Overtime"}>
                      <button
                        id="overtime-button"
                        onClick={e => {
                          e.stopPropagation();
                          navigate(`/overtime/${row._id}`);
                        }}
                        className=""
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="#f8f8f8" className="size-4 hover:size-5 hover:scale-105 duration-300 active:scale-95 cursor-pointer">
                  ...
                  {/* Equip */}
                  {/* {canEquip && (
                    <Tooltip position="bottom" content={"Equipment"}>
                      <button
                        id="equip-button"
                        onClick={e => {
                          e.stopPropagation();
                          navigate(`/equipment/${row._id}`);
                        }}
                        className=""
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="#f8f8f8" className="size-4 hover:size-5 hover:scale-105 duration-300 active:scale-95 cursor-pointer">
                  ...
                  {/* Bonus */}
                  {isFinance && (
                    <Tooltip position="bottom" content={"Bonus"}>
                      <button
                        id="bonus-button"
                        onClick={e => {
                          e.stopPropagation();
                          navigate(`/bonus/${row._id}`);
                        }}
                      >
                        {loadingId === row._id ? (
                          <svg
                  ...
                  {/* Invoice */}
                  {isFinance && (
                    <Tooltip position="bottom" content={"Invoice"}>
                      <button
                        id="bonus-button"
                        onClick={e => {
                          e.stopPropagation();
                  ...
                  {/* Quotation */}
                  {isFinance && (
                    <Tooltip position="bottom" content={"Quotation"}>
                      <button
                        id="bonus-button"
                        onClick={e => {
                          e.stopPropagation();
                          navigate(`/quotation/${row._id}`);
                        }}
                      >
                        {loadingId === row._id ? (
                          <svg
