...
const EquipmentWrapper = ({ data, updateData }) => {
  const { id } = useParams();
  const [project, setProject] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        ...
        // First try to find in local data (for faster initial load)
        const found = data.find(d => d._id === id);
        // For Equipment page, we need equipment data, so check if it exists
        // The fetch/fetchArchived endpoints exclude equipment field for performance
        if (found && found.day && found.equipment !== undefined) {
          // If we have full data locally (including equipment), use it
          setProject(found);
          setLoading(false);
          return;
        }
        // Otherwise fetch full project data from API (which includes equipment)
        const response = await axios.get(`${apiUrl}/getproject/${id}`);
        setProject(response.data);
      } catch (err) {
        console.error("Error fetching project:", err);
