...
const PDFInvoice = ({ pro, days, invoiceNumber, invoiceDate, dueDate, subtotal, taxRate, taxAmount, total, notes }) => {
  ...
  // Collect all expense items from all days
  const allItems = [];
  days.forEach((day, dayIndex) => {
    ...
    if (day.expense?.rent && Array.isArray(day.expense.rent)) {
      day.expense.rent.forEach(item => {
        if (item.name && item.price && item.qty) {
          allItems.push({
            description: `${item.name} (${dayLabel})${item.note ? ` - ${item.note}` : ''}`,
            qty: item.qty || 0,
            price: parseFloat(item.price || 0),
            total: (item.qty || 0) * parseFloat(item.price || 0)
          });
        }
      });
    }
    ...
    if (day.expense?.operational && Array.isArray(day.expense.operational)) {
      day.expense.operational.forEach(item => {
        if (item.name && item.price && item.qty) {
          allItems.push({
            description: `${item.name}${item.category ? ` (${item.category})` : ''} (${dayLabel})${item.note ? ` - ${item.note}` : ''}`,
            qty: item.qty || 0,
            price: parseFloat(item.price || 0),
            total: (item.qty || 0) * parseFloat(item.price || 0)
          });
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.headerwrap}>
          ...
          <View style={styles.invoiceInfo}>
            <View style={styles.infoRow}>
              ...
              <Text style={styles.infoValue}>{invoiceNumber || "INV-XXXX"}</Text>
            </View>
            <View style={styles.infoRow}>
        ...
        <View style={styles.footer}>
          ...
          <Text style={{ marginTop: 5 }}>
            Invoice generated on {new Date().toLocaleDateString("id-ID", {
              day: "2-digit",
              month: "long",
              year: "numeric"
            })}
